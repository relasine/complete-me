const Node = require('../lib/Node')

class Trie {
  constructor() {
    this.root = null;
    this.wordCount = 0;
  }

  count() {
    return this.wordCount;
  }

  insert(newWord) {
    this.wordCount ++

    let wordArray = newWord.split('');

    if (!this.root) {
      this.root = new Node();
      this.root.letter = 'Node'
    }

    let currNode = this.root;

    wordArray.forEach((arrLetter) => {
      if (!currNode.children[arrLetter]) {
        currNode.children[arrLetter] = new Node(arrLetter);
        currNode = currNode.children[arrLetter]
      } else if (currNode.children[arrLetter]) {
        currNode = currNode.children[arrLetter]
      }
    });

    currNode.wordEnding = newWord;
  }






  suggest(word) {
    let wordArray = word.split('');
    let currNode = this.root;

    wordArray.forEach((arrLetter) => {
      if (currNode.children[arrLetter]) {
        currNode = currNode.children[arrLetter];
      }
    });

    let startNode = currNode;
    let currKeys = Object.keys(currNode.children)

    let suggestions = currKeys.reduce((suggestionArray, key) => {
      let loopSug = Object.keys(currNode.children[key].children).forEach(() => {
        suggestionArray.concat(this.pullSuggestions(currNode,suggestionArray))
      })

      suggestionArray.concat(loopSug)

      return suggestionArray
    }, []);

    console.log(suggestions)

    return suggestions
  }






  pullSuggestions(node, array) {
    let currNode = node;
    let suggestionArray = array;

    currNode = node;
    // if (currNode.children) {
    //   (Object.keys(currNode.children)).forEach((key) => {
    //     console.log(currNode.children[key])
    //   })
    // }

    Object.keys(currNode.children).forEach((child) => {
      console.log(currNode)
      if (currNode.children[child]) {
        currNode = currNode.children[child]
      }
      if (currNode.wordEnding !== null) {
        suggestionArray.push(currNode.wordEnding)
      }
      console.log('2', currNode)
    })


    if (currNode.wordEnding !== null) {
      suggestionArray.push(wordEnding);
    }

    // suggestionArray.push('yo')

    return suggestionArray;
  }


  // populate(dictionary) {
  //   dictionary.forEach((word) => {
  //     this.insert(word)
  //   });
  // }
}




module.exports = Trie;